...
class Awakening(commands.Cog):
    ...
    async def awakening_group(self, ctx):
        embed = discord.Embed(title="Awakening command info", color=0xC71585)
        embed.add_field(name="", value="* ?awk list 로 목록을 확인할 수 있어요.")
        embed.add_field(name="", value="* ?awk info <name> 으로 정보를 확인할 수 있어요.")
    ...
    async def awakening_list(self, ctx, *args):
        ...
        if len(args) > 0:
            if args[0] == "all":       # ?awk list all
                ...
                embed = discord.Embed(title="Awakening List in Rotation", color=0xC71585)
